...
最后一个也最关键的原因，在于捍卫《垦殖智域》中提到的同行评审和礼物文化的运作模式。旨在保卫知识产权或获取直接销售价值的许可限制，往往在法律上断绝了fork<sup>c</sup>项目的可能性。Sun公司针对Jini和Java的所谓"社区源代码"许可证就属于这种情况。虽然大家普遍对fork行为嗤之以鼻，也认为这种行为是万不得已之举（原因在《开拓智域》中有详细论述），但是当维护者无能或背刺（例如转向更封闭的许可证）时<sup>5</sup>，保留这个最后的手段至关重要。
